    ...
    async def chat_completions():
        ...
        for record in batch_results:
            ...
            if parsers:
                ...
                # Normalize tool_calls to OpenAI shape (id, type, function.name, function.arguments string)
                # for downstream clients. When the client did not request tools (plain chat), do not
                # expose parser-detected tool syntax as tool_calls; keep raw text and drop tool_calls.
                if "tool_calls" in metadata:
                    metadata["tool_calls"] = _normalize_tool_calls(
